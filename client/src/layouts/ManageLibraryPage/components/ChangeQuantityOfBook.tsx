...
const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: any }> = (props) => {
    ...
    async function increaseQuantity() {
        ...
        if (!quantityUpdateResponse.ok) {
            throw new Error('Something went wrong. Unable to increase book quantity.');
        }
    ...
    async function decreaseQuantity() {
        ...
        const quantityUpdateResponse = await fetch(url, requestOptions);
        if (!quantityUpdateResponse.ok) {
            throw new Error('Something went wrong. Unable to decrease book quantity.');
        }
        setQuantity(quantity - 1);
        setRemaining(remaining - 1);
    }
    ...
    async function deleteBook() {
        ...
        if (!updateResponse.ok) {
            throw new Error('Something went wrong. Unable to delete book.');
        }
        props.deleteBook();
    }
